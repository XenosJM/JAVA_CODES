...
//				int size = ((ContactDAOImple) dao).getSize(); // 내 코드
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					System.out.println("contactId 입력완료");
//					ContactVO vo = (dao.select(index));
					if (contactId >= 0 && contactId <= ((ContactDAOImple) dao).selectDB(contactId).getContactId()) {
						System.out.println("검색 성공");
						textAreaInfo.setText(((ContactDAOImple) dao).selectDB(contactId).toString());
					} else {
...
//				int size = ((ContactDAOImple) dao).getSize();
//				if(contactId >= 0 && contactId < size) {
				int result = ((ContactDAOImple) dao).deleteDB(contactId);
				if (result == 1) {
		...
		/*
		 * 1. 정보를 입력하고 등록버튼을 클릭하면 정보가 파일에 저장된다. 2. 인덱스 번호를 입력하고 검색 번호를 누르면 파일에 저장된 하나의
		 * 데이터가 출력한다. 3. 인덱스 번호를 입력하고 정보를 입력한 후, 수정버튼을 누르면 파일에 저장된 정보가 수정된다. 4. 인덱스 번호를
		 * 입력하고 삭제버튼을 누르면 파일에 저장된 정보가 삭제된다. 5. 전체검색을 클릭하면 모든 정보가 출력된다.
		 * 
		 * 추가 1. 수정버튼을 누르면 전체 데이터를 테이블에 출력한다. 2. 삭제버튼을 누르면 전체 데이터를 테이블에 출력한다. 3. 전체검색을
		 * 누르면 전체 데이터를 테이블에 출력한다.
		 * 
		 * TODO 기능들 모듈화 해서 밑에 메소드화 해서 빼놓고 실행해보기
		 */
		/*
		 * private String[] colNames = private Object[] records = new Object[colums.
		 * mode.addRow(data);
		 */
